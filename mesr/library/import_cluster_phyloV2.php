...
//$fichier=$adresse_root.'/'.$dbid."/".$exportid."/".$fichier; Correction David suite pb import
echo $fichier;
	...
	for ( $j = 1 ; $j < (count($ligne_v)/3-1) ; $j++ )
		{
		  $periode_2 =  $ligne_v[3*$j].' '.$ligne_v[3*$j+1];
			$id_cluster_2 = intval($ligne_v[3*$j+2]);
