...
echo '<form action="entree_clusters_generate_lettres.php" method="get" style="display:inline;">';
...
for ($i=0;$i<count($liste_termes_brute);$i++){
	...
	$arrows = '';
	if (intval($pere[$i])>0) $arrows.='&uarr;';
	if (intval($fils[$i])>0) $arrows=$arrows.'&darr;';	
...
foreach ($groupes_clusters as $key => $value) {
	if (count($value["homos"])>1) {
		...
		for ($i=0;$i<count($value["homos"]);$i++){
			...
			$sql_update = "UPDATE  cluster SET lettre ='".$letter."' where id_cluster=".$h["id"]." AND periode='".derange_periode($my_period)."'";
