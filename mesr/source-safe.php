...
//		echo $sql;
	...
	//print_r($liste_auteur_unique);echo("<br>");
	//print_r($legende);echo("<br>");
	//print_r($liens_from);echo("<br>");
	//print_r($liens_to);echo("<br>");
	//print_r(array_flip($liste_auteur_unique));echo("<P>");
	...
	{
		$tmp["nodes"][]=array("nodeName" => $legende[$i], "group" => 0, "ourID" => $i);
	}
	...
	foreach(range(0,count($liens_from)-1) as $i)
	{
		...
		if (!array_key_exists($tmpfrom,$tmpdico)) {$tmpdico[$tmpfrom]=array();}
		if (!array_key_exists($tmpto,$tmpdico[$tmpfrom])) {$tmpdico[$tmpfrom][$tmpto]=0;}
		$tmpdico[$tmpfrom][$tmpto]+=1;
	}
	foreach(array_keys($tmpdico) as $i) {
		foreach(array_keys($tmpdico[$i]) as $j) {
				$tmp["links"][]=array("source" => $i, "target" => $j, "value" => $tmpdico[$i][$j]); 
			}
		}
	...
	if ($nopred==0)
	...
	else
	  {
		...
		if ($my_period!=-1) echo '<a href=source.php?id_source='.$_GET['id_source']."&periode=".$persuivante.'>RÉSEAU À LA PÉRIODE PRÉCÉDENTE</a>';
		echo '<br>';
		echo '</td>';
		echo '</tr>';
	...
	// AFFICHAGE RESEAU FLEX
	echo '<script language="JavaScript" type="text/javascript">';
	echo '<!--
		// Major, Minor, Minor version of Flash required
	...
	if ($nosucc==0) 
		...
		echo '<table class=commentitems align=center width=100% cellspacing=0  cellpadding=5 style="font-variant:small-caps; size:small; font-style:italic;">';
			...
			echo '<td align=right>';
			$persuivante = strval(intval($periodex["from"])+7).'-'.strval(intval($periodex["to"])+7);
			if ($my_period!=-1) {echo '<a href=source.php?id_source='.$_GET['id_source']."&periode=".$persuivante;
			echo '>';
